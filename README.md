...
6. Run Title/Author Review. Extraction, the verification image, title comparison, and authors are reviewed together; `Review OK` is the single completion state. Built-in, GROBID, and Manual Override results use the same collision-safe verification renderer: a separate review header lists the source, filename, extracted title, and numbered authors, while the PDF evidence below uses title underlines and separate author boundaries. Use optional GROBID fallback only for suspicious rows or individual papers that the built-in extractor handles poorly. Use Manual override only as a documented exception when extraction cannot be fixed through formatting/re-extraction.
