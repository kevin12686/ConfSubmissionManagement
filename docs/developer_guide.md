...
- Heavy worklist evidence should not be eagerly decoded or rendered. Title/Author verification images use native lazy loading and dimensions read from the PNG header; do not decode each image just to size the worklist. Built-in, GROBID, and Manual Override must all use `submissions/services/title_author_verification.py`; extractor-specific renderers are not allowed. The renderer keeps its dynamic review header outside the PDF evidence region so long titles, filenames, and author legends cannot overlap source content. Each Manual Override form is fetched only when its collapsed panel is opened. The partial endpoint is read-only; the existing audited POST workflow remains the only mutation path.
