...
- Heavy worklist evidence should not be eagerly decoded or rendered. Title/Author verification images use native lazy loading with fixed intrinsic dimensions, and each Manual Override form is fetched only when its collapsed panel is opened. The partial endpoint is read-only; the existing audited POST workflow remains the only mutation path.
